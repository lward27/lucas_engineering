...
usage() {
  echo "usage: $0 [--repo PATH] [--remote NAME] [--branch NAME] [--revision 40_CHAR_SHA]" >&2
  exit 2
}
...
if [[ -n "$REQUESTED_REVISION" && ! "$REQUESTED_REVISION" =~ ^[0-9a-f]{40}$ ]]; then
  echo "requested revision must be a full lowercase 40-character Git commit SHA" >&2
  exit 1
fi
...
[[ "$FETCHED_REVISION" == "$REMOTE_REVISION" ]] || {
  echo "remote branch moved during revision verification; retry from a fresh preflight" >&2
  exit 1
}
